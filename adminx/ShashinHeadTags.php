...
class Admin_ShashinHeadTags {
    ...
    public function run() {
        $cssUrl = $this->functionsFacade->getUrlforCustomizableFile('admin.css', __FILE__, 'displayx/');
        $this->functionsFacade->enqueueStylesheet('shashinAdminStyle', $cssUrl, false, $this->version);
        $jsUrl = $this->functionsFacade->getUrlforCustomizableFile('admin.js', __FILE__, 'displayx/');
        $this->functionsFacade->enqueueScript('shashinAdminScript', $jsUrl, array('jquery', 'jquery-ui-tabs'), $this->version);
        $menuDisplayUrl = $this->functionsFacade->getPluginsUrl('/displayx/', __FILE__);
